...
Thread::Thread(char* threadName)
{
	...
	***********/
	...
	if (tid == -1) 
		printf("Max thread number limitation reached.\n");
	ASSERT(tid >= 0);
...
Thread::~Thread()
{
    ...
    if (stack != NULL)
	DeallocBoundedArray((char *) stack, StackSize * sizeof(int));  // Relinquish stack space
	/***********
	new code begins
	***********/
	TidStatus[tid] = 0;
...
//	atomically.  On return, we re-setthe interrupt level to its
